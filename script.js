...
async function handleFileUpload(event) {
  ...
  try {
    ...
    formData.append(
      "public_id",
      `3d_models/${Date.now()}_${file.name.replace(/\.[^/.]+$/, "")}`
    ); // Custom filename
    ...
    if (result.secure_url) {
      ...
      const originalFilename = result.public_id
        .split("/")[1]
        .split("_")
        .slice(1)
        .join("_"); // Extract filename
      ...
      showNotification(
        `${file.name} uploaded successfully! (${formatFileSize(file.size)})`
      );
      updateGallery();
    } else {
  ...
  try {
    ...
    if (scene) {
      ...
      if (
        ...
        scene.extensionsUsed.includes("KHR_materials_pbrSpecularGlossiness")
      ) {
        console.info(
          "Model uses PBR Specular-Glossiness materials - this is normal"
        );
      }
...
function onModelError(error) {
  ...
  if (
    ...
    console.warn(
      "Model uses PBR Specular-Glossiness materials (non-critical warning)"
    );
    return;
  }
...
function generateQRCode(modelId) {
  ...
  try {
    ...
    showNotification(
      "QR code generated! Share this link to view the model on any device."
    );
  } catch (error) {
...
async function handleFileUploadImproved(event) {
  ...
  try {
    ...
    // Get file extension
    const fileExtension = file.name.toLowerCase().endsWith(".glb")
      ? ".glb"
      : ".gltf";
...
async function checkUrlParameters() {
  ...
  if (modelId) {
      ...
      try {
        ...
        if (modelData.url && modelData.url.includes("cloudinary.com")) {
          ...
          if (urlTest.success) {
            ...
            console.warn(
              "Stored model URL is no longer accessible:",
              urlTest.error
            );
    ...
    if (modelId.includes("model_")) {
      ...
      showLoading(false);
      showNotification(
        "Model not found or no longer available. The link may have expired.",
        "error"
      );
  ...
  item.innerHTML = `
        <div class="gallery-item-header">
            <div class="gallery-item-info">
                ...
                <div class="file-meta">${
                  modelData.fileSize || "Unknown size"
                } • ${uploadDate}</div>
            </div>
        </div>
...
async function loadModelFromGallery(modelId) {
  const modelData =
    models.get(modelId) || JSON.parse(localStorage.getItem(`model_${modelId}`));
...
async function deleteModelFromGallery(modelId) {
  ...
  try {
    ...
    if (modelData.cloudinaryId) {
      ...
      console.log(
        "Model should be deleted from Cloudinary:",
        modelData.cloudinaryId
      );
